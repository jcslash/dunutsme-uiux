...
import { GoogleIcon, AppleIcon, MailIcon, UserCircleIcon, CloseIcon } from './visuals/Icons';
...
// Static arrow icon (hoisted outside)
const ArrowIcon = (
...
const SocialButton = memo<{
  ...
  badge?: string;
  showArrow?: boolean;
}>(({ icon: Icon, label, onClick, badge, showArrow }) => (
  <button 
    type="button" 
    onClick={onClick} 
    className="flex items-center justify-between w-full h-[52px] px-4 rounded-[16px] border border-[#E5E7EB] hover:bg-[#F9FAFB] transition-all group"
  >
    <div className="flex items-center gap-3">
      <Icon className="w-5 h-5" />
      <span className="font-dm-sans font-medium text-gray-700">{label}</span>
    </div>
    {badge && (
      <span className="text-[10px] font-bold text-[#4B5563] bg-[#F3F4F6] px-1.5 py-0.5 rounded text-center uppercase tracking-wider">
        {badge}
      </span>
    )}
    {showArrow && (
...
export const LoginModal: React.FC<LoginModalProps> = memo(({ isOpen, onClose, onLoginSuccess }) => {
        ...
        <div className="flex justify-center mb-8">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 bg-black rounded-full flex items-center justify-center relative overflow-hidden">
              <div className="absolute top-[30%] w-[60%] h-[30%] bg-white/20 rounded-full" />
            </div>
